...
/* The policy consumes the last painted identity, not merely an allowed paint. */
{
	...
	assert.strictEqual(s.shouldRender(rows(['a']), false, 99), true, 'count changes cost one');
	assert.strictEqual(s.shouldRender(rows(['a']), false, 99), true, 'decision does not commit identity');
	s.markRendered(rows(['a']));
	assert.strictEqual(s.shouldRender(rows(['a']), false, 99), false, 'unchanged rows skip painting');
	assert.strictEqual(s.shouldRender(rows(['b']), false, 2), false, 'new IDs charge the batch cost');
	assert.strictEqual(s.isFloodSuppressed(), true);
	assert.strictEqual(s.shouldRender(rows(['b']), true, 99), true, 'forced paint bypasses the bucket');
	assert.strictEqual(s.isFloodSuppressed(), true, 'forced paint preserves the warning');
	assert.strictEqual(s.shouldRender(rows(['a']), false, 99), false);
	assert.strictEqual(s.isFloodSuppressed(), false, 'unchanged rows clear the warning');
	assert.strictEqual(s.shouldRender(rows([]), false, 99), true, 'clearing the table costs one');
	s.markRendered([]);
	assert.strictEqual(s.shouldRender([], false, 99), false, 'empty table remains unchanged');
}
...
/* Recovery uses elapsed time, including clocks whose first timestamp is zero. */
{
	...
	s.markRendered(rows(['a']));
	assert.strictEqual(s.shouldRender(rows(['b']), false, 3), true);
	s.markRendered(rows(['b']));
	assert.strictEqual(s.shouldRender(rows(['c']), false, 1), false);
	time = 500;
	assert.strictEqual(s.shouldRender(rows(['c']), false, 1), true, 'half a second refills 1.5 tokens');
	s.markRendered(rows(['c']));
	assert.strictEqual(s.isFloodSuppressed(), false, 'budget recovery clears the warning');
	time = 400;
	assert.strictEqual(s.shouldRender(rows(['d']), false, 1), false, 'clock rollback grants no tokens');
	time = 600;
	assert.strictEqual(
	...
	assert.strictEqual(s.shouldRender(rows(['f']), true, 3), true);
	assert.strictEqual(s.shouldRender(rows(['f']), false, 1), false, 'forced paint does not refill');
	time = 10500;
	s.resetBudget();
	assert.strictEqual(s.shouldRender(rows(['g']), true, 99), true);
	time = 11000;
...

{
	...
	h.scheduler.schedule(false);
	assert.strictEqual(h.frames.length, 1, 'requests coalesce even when frame ID is zero');
	h.flush();
...
for (const force of [false, true]) {
	...
	assert.strictEqual(h.frames.length, 1, 'latest request gets a fresh frame');
	h.flush();
	assert.deepEqual(
		h.renders,
		[force],
...

{
	...
	assert.strictEqual(h.frames.length, 0);
	h.scheduler.schedule(false);
	h.flush();
