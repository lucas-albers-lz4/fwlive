...
# --- @zone[N] vs cfgXXXX in uci changes (false firewall_changes_pending) ---
...
case "$out" in
	*'"ok":true'*)
		ok "enable succeeds when uci changes resolves zone to cfg id"
		;;
	*'"error":"firewall_changes_pending"'*)
		ok "repro: false firewall_changes_pending when zone is @zone[0] but changes use cfg id"
		;;
	*) die "unexpected enable output: $out" ;;
esac
[ "$UCI_COMMITTED" = "1" ] || ok "repro: enable aborted before commit (cfg vs @zone id mismatch)"
rm -rf "$ZONE_MISMATCH_WORK"
unset WAN_LOG_BASELINE_FILE PENDING_STAGED UCI_COMMITTED CURRENT_LOG ZONE_MISMATCH_WORK
