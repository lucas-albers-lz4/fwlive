...
| **Enable logging** | Filled button on the watch strip when WAN logging is off. Sets WAN zone drop/reject logging (same as Network → Firewall). No allow/deny rules are added. Concurrent toggles from multiple admins serialize under an exclusive file lock (`flock`): waiters block until the holder releases the lock after the short read→commit section, not until the whole RPC including firewall reload finishes. Then each toggle re-reads WAN state before committing (not a lost-update race). If a toggle appears to hang, the lock may be held — not a silently dropped write. Rate is the firewall `log_limit` (default `10/minute`), not a fwlive cap. |
