...
| **Package output** | `aarch64_generic`, `x86_64` | Under `out/<arch>/<patch>/fwlive/` (patch label from `sdk_matrix_version_label`, e.g. `24.10.8`) |
...
1. Guest `css.js` includes scoped `--fwlive-pass-color` / Material `var(--success-color, …)` (classic) and fixed accessible teal/orange hex (`#0d9488` / `#c2410c`) / rgba bases and `--fwlive-bg-medium` / `var(--white-color-low, …)` for zebra
