...
| Empty filter output on `poll` returns `error:filter_empty` | `code` | Defensive guard (line 437): `[ -n "$out" ] || out='{"log":[],"error":"filter_empty"}'`. The shipped filter always prints `{"log":[…]}`, so this is fail-closed over a class of stub/missing-filter regression; no host test induces it without replacing the shipped filter, which is out of scope. Neighbouring `filter_failed` pinned by `tests/fwlive-view-poll-error.test.js`. |
