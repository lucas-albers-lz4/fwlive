...
> **#392 R4b:** the installed LuCI/uhttpd session smoke passed on OpenWrt 24.10.8 x86_64. A temporary account with the `luci-app-fwlive` ACL called all six declared fwlive methods, was denied `log.read` with JSON-RPC code `-32002`, and an authenticated `luci-base` account without the app grant was denied fwlive calls with the same code. The artifact records the package/source and installed file hashes; cleanup reverts staged UCI state, restores and hash-verifies `/etc/config/rpcd`, and restores WAN logging before removing the backup.
...
| Authenticated LuCI/uhttpd sessions receive fwlive read/write methods only when the `luci-app-fwlive` ACL is granted | `lab` | `scripts/qemu-acl-session-smoke.sh`; [#392 installed-session artifact](../evidence/issue-392-2026-09-20.md) |
