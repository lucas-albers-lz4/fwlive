...
| Adaptive state at `/var/run/fwlive-state.json` — owner/dir checks, atomic rename, non-blocking `flock -n` on sibling `.lock` (fail-open; release by closing fd; busy ⇒ last-writer-wins), never `source`/`eval` | `host` | same; lock only around short update, never across ubus/filter |
| Hot shed surfaces `shed` + `truncated`; `resolve` returns `disabled:load` when previous poll was hot; failed `log.read` does not clear prior bucket | `host` | `tests/fwlive-rpcd-security.test.js` `testAdaptiveHotSurvivesFailedPoll`; adaptive helper + rpcd skips `record` on `log_read_failed` |
...
| `resolve` without `jshn` / with malformed input returns `error:jshn_missing` / `error:invalid_input` | `host` | same file `testResolveJshnMissing`; `invalid_input` runs only where `jshn` exists (skips on stock hosts). UI consumption is out of scope here: `callFwliveResolve` uses `expect: { names: {} }` (#243) which drops sibling `error` — the existing `no_resolver` pre-check still reaches the view, but the new fail-closed codes are an rpcd contract, not a UI one. |
| `logging_status` always returns the full 8-key shape; failures travel as `blockers`/`warnings` with `ready:false`, never a silent empty object | `host` | same file `testLoggingStatusNeverSilent` |
