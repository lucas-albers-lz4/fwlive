...
> **#392 R4b:** the installed LuCI/uhttpd session smoke passed on OpenWrt 24.10.8 x86_64. A temporary account with the `luci-app-fwlive` ACL called four selected fwlive methods, while an authenticated account without the grant was denied `logging_status` and `enable_wan_logging` with JSON-RPC code `-32002`. The artifact records the package/source and installed file hashes, and cleanup restored the original rpcd config and WAN logging state.
