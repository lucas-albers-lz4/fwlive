...
> **Current delta:** 2026-09-19 #373 WAN-zone resolution by exact name or effective `wan`/`wan6` network membership, with JSON-escaped no-zone candidates; host-focused validation; no ACL or DOM-sink change.
...
| WAN-zone lookup accepts exact `name=wan` or effective `network=wan`/`wan6`, preserves config order, and reports discovered zone names when no supported zone matches | `host` | `tests/fwlive-logging.test.sh` #373 scalar/list/renamed/duplicate/no-match fixtures; qemu helpers consume `logging_status.wan_zone` |
