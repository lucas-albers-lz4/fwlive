...
| Forwarding-SLO lab topology owns only `fwlive-slo-*` network namespaces, bridges, TAPs, and veths; teardown validates the prefix before removing them; no shipped surface or runtime ACL is changed | `host` | `scripts/qemu-forwarding-slo-net.sh` / `scripts/lib/qemu-forwarding-slo-net.sh`; static shell checks and manual lab verification in #344 |
| Forwarding-SLO guest helper resolves interfaces by caller-supplied MAC, uses fixed temporary forwarding/logging rule comments, validates rule handles as digits during cleanup, and changes no shipped package or ACL | `host` | `scripts/qemu-forwarding-slo-guest.sh`; shell syntax/ShellCheck plus manual armsr verification in #344 |
| Forwarding-SLO traffic probe requires root-owned named namespaces, validates positive bounded duration/count inputs and a safe label, uses an absolute `iperf3` path, and emits measurements without changing runtime state | `host` | `scripts/qemu-forwarding-slo-traffic.sh`; shell syntax/ShellCheck plus routed `iperf3`/ping probe in #344 |
| Forwarding-SLO paired runner scopes the adaptive sentinel to the selected guest, uses a separate local Playwright worker with marker-file IPC, drains the worker before recording, and writes reports only to the caller-selected path | `host` | `scripts/qemu-forwarding-slo-run.sh`, `tests/fwlive-forwarding-slo-viewer.mjs`; shell syntax/ShellCheck/Node checks plus one-pair armsr smoke in #344 |
