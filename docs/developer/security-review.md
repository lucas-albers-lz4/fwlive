...
| Forwarding-SLO guest helper resolves interfaces by caller-supplied MAC, uses fixed temporary rule comments, validates rule handles as digits during cleanup, and changes no shipped package or ACL | `host` | `scripts/qemu-forwarding-slo-guest.sh`; shell syntax/ShellCheck plus manual armsr verification in #344 |
