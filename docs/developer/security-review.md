...
| Adaptive poll cap always on; no UCI; disable only via `FWLIVE_ADAPTIVE=0` or sentinel file | `host` | `tests/fwlive-adaptive-cap.test.sh`; reply carries `adaptive` |
| Adaptive state at `/var/run/fwlive-state.json` — owner/dir checks, atomic rename, non-blocking `flock -n` on sibling `.lock` (fail-open; release by closing fd), never `source`/`eval` | `host` | same; lock only around short update, never across ubus/filter |
| Hot shed surfaces `shed` + `truncated`; `resolve` returns `disabled:load` when previous poll was hot | `host` | same + `fwlive_adaptive_is_hot` |
