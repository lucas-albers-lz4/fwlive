...
> **#392 R4b:** the installed LuCI/uhttpd session smoke passed on OpenWrt 24.10.8 x86_64. A temporary account with the `luci-app-fwlive` ACL called all six declared fwlive methods, was denied `log.read` with JSON-RPC code `-32002`, and an authenticated `luci-base` account without the app grant was denied fwlive calls with the same code. The artifact records the package/source and installed file hashes, and cleanup restored the original rpcd config and WAN logging state.
