...
| Hot shed surfaces `shed` + `truncated`; `resolve` returns `disabled:load` when previous poll was hot; failed `log.read` does not clear prior bucket | `host` | same + `fwlive_adaptive_is_hot`; rpcd skips `record` on `log_read_failed` |
