...
| #370 package payload | 2026-09-21 | Delta + host test | Required per-PR job builds current-version 23.05 IPK, 24.10 IPK, and 25.12 APK artifacts and runs the format-aware payload inspector and lifecycle tests with `FWLIVE_REQUIRE_PACKAGE=1`; it checks JS modules, ACL/menu files, libexec layout, executable modes, and packaged lifecycle contracts. IPK `prerm-pkg` cases are executed; APK data-only extraction does not execute the APK hook. |
