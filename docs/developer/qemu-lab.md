...
Sets LAN **dhcp** for slirp, relaxes lab firewall, empty root password, dropbear auth, uhttpd/LuCI fixes. **22.03.x** fresh x86 images may lack `/etc/config/network` — the script seeds DHCP `lan` before first boot (see [22.03 notes](../supported-releases.md#2203x-historical)):
...
`OWRT_QEMU_SMP=1 OWRT_QEMU_MEM=256`. The guest reported revision
`r29233-443ec4032a`, package `luci-app-fwlive 0.1.41-r1`,
