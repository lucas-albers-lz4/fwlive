#!/usr/bin/env bash
# Register fwlive feed and install luci-app-fwlive inside the SDK in the Docker volume.
# Uses /work/fwlive/openwrt-feed (repo mounted read-only in compose).
set -euo pipefail
...
docker compose run --rm sdk-legacy sh -ec '
	...
	if ! grep -q "^src-link fwlive" feeds.conf 2>/dev/null; then
		echo "src-link fwlive /work/fwlive/openwrt-feed" >> feeds.conf
	fi
