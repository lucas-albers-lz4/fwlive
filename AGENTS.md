...
- CodeRabbit automatically reviews only Ready PRs (`auto_review.drafts: false`); manual `@coderabbitai review` can still trigger on drafts. Review limits are **plan-specific rolling allowances** (Free 1/hr, Pro 5/hr, Pro+ 10/hr — check quota with `@coderabbitai rate limit`), not a fixed ~3/hr cap. Marking Ready makes the PR *eligible* for automatic review. Wait for a round to complete before pushing fixes (a rate-limit comment means the head was NOT reviewed), batch fixes into one push, don't declare the gate green mid-round. [coderabbit.md](docs/developer/coderabbit.md)
