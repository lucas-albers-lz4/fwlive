...
- Cross-repo auto-close trap: never pair a GitHub closing keyword (`fix`/`fixes`/`closes`/`resolves`/…) with `owner/repo#N` (or a github.com URL) in **any** fwlive PR title, body, or commit text — that closes the foreign PR (hit by #255 squash on openwrt/luci#8992). Use `Refs owner/repo#N` only. Full rules + reply≠resolve + CI `action_required`: [upstream-openwrt.md](docs/developer/upstream-openwrt.md#upstream-merge-safety).
